...
3. Validate and do a small live run:
   ...
   ```
4. Record the source in the `data/sources/additional_master_sources.xlsx` **outbox** (its `source_id` and the
...
5. Open a pull request. Do **not** commit scraped output (`data/scraped/` is gitignored) — only the
   recipe and, if you added one, the `additional_master_sources.xlsx` outbox row.
